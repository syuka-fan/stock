...
| 파일 | 역할 |
| --- | --- |
| [symbols.json](symbols.json) | 추적 대상 티커 정의 (us / kr / index) |
...
| [build_index.py](build_index.py) | `template.html`에 데이터 임베드, PWA 메타·SW·SWR 부트스트랩 주입 |
| [template.html](template.html) | 대시보드 UI 템플릿 (치환 앵커 포함) |
...
python build_index.py             # index.html 빌드
```
...
1. `download.py` 실행 후 워킹트리 변화가 있거나 분석 출력 디렉터리 중 하나라도 비어있으면 게이트 통과
2. `correlation_analysis.py` / `direction_analysis.py` 병렬 실행
3. 결과를 `auto/update-<KST타임스탬프>` 브랜치로 push → PR 생성 → squash 머지
4. 머지된 main 기준으로 `build_index.py` 실행 후 GitHub Pages로 배포
